...
Os algoritmos fornecidos foram: K-means, Color Histogram e Eternity II. A descrição original do trabalho e os arquivos .C com os códigos seriais fornecidos pelo professor estão disponíveis AQUI, bem como as versões paralelas propostas.
...
O sistema operacional da máquina é um Linux Ubuntu Release 18.04. Já o compilador utilizado foi o gcc na versão 7.5.0. As saídas dos comandos `lscpu`, `gcc --version` e `lsb_release -a` estão disponíveis AQUI.
...
Através da Lei de Amdahl podemos obter o speedup teórico antes mesmo da realização do experimento. A ideia consiste em analisar o tempo total da aplicação nos trechos que serão mantidos como sequenciais e dividir o tempo do trecho que será paralelizado por valores candidatos a número de processadores.
...
A eficiência mede o grau de aproveitamento dos recursos utilizados, isto é, esperamos que a eficiência aumente n vezes sendo n o número de threads. Podemos observar uma queda de eficiência conforme aumentamos o número de threads.
